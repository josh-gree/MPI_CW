...
#define FILENAME "edge512x384.pgm"
...
int main (int argc, char **argv)
{
	...
	char filename2[64];
	char filename3[64];
	sprintf(filename1,"out_%d.dat",P);
	sprintf(filename2,"times_%d.dat",P);
	FILE *fp_outres; 
	FILE *fp_times;
	...
	double time1, time2, time1com, time2com;
	float totalcommtime = 0.0;
	float commtime[MAXITER];
	for (i = 0;i<MAXITER;i++){
		commtime[i] = 0.0;
	}
	...
	for(iter = 0;iter < MAXITER;iter++){
	    ...
	    time2com = MPI_Wtime();
	    commtime[iter] = time2com - time1com;
	...
	for (i = 0;i<iter;i++){
		totalcommtime += commtime[i];
	}
	// print time to file
	if (rank ==0 )fprintf(fp_times,"%d,%f,%f,%f\n",P,time2 - time1,(time2 - time1)/(double) iter,totalcommtime/(double) iter);
	...
	sprintf(filename3,"image_%d.pgm",P);
	// write image to disk
	if (rank == 0){
